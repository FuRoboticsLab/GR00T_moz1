...
def main(args: ArgsConfig):
    ...
    print("All trajectories (lengths):", dataset.trajectory_lengths)
    print("All trajectory_ids (episode_index):", dataset.trajectory_ids)
    print("Running on traj indices with modality keys:", args.modality_keys)

    all_mse = []
    # NOTE:
    #   dataset.trajectory_ids 存的是 episodes.jsonl 里的 episode_index，
    #   在子集数据集（例如只保留成功 episodes）里，这些索引可能不是连续的 0..N-1。
    #   因此这里用「轨迹下标 idx」映射到真正的 episode_index，再传给 calc_mse_for_single_trajectory。
    for idx in range(args.start_traj, args.start_traj + args.trajs):
        if idx >= len(dataset.trajectory_ids):
            print(f"Requested traj index {idx} >= total trajectories {len(dataset.trajectory_ids)}, stop.")
            break

        traj_id = int(dataset.trajectory_ids[idx])
        print(f"Running trajectory index {idx} (episode_index={traj_id})")
